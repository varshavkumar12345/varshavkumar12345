## Hi there 👋 I'm Varsha V Kumar

I'm an under grad studying Artificial Intelligence and Machine Learning at BMS College of Engineering. Currently focusing on LLM security. 🦠

---

## 🧰 Tech Stack

**Languages:**  
`Python` · `Java` · `C` · `C++`

**Frameworks & Libraries:**  
`Pandas` · `scikit-learn` · `Transformers` · `Keras` · `TensorFlow` · `Flask` · `Matplotlib` · `Seaborn` · `Statsmodels` · `SciPy` · `Sympy` · `OpenCV` · `AstroPy`

**Databases:**  
`MongoDB` · `MySQL` · `ChromaDB`

**Tools & Platforms:**  
`Git` · `GitHub` · `Docker` · `Linux` · `Render` · `AWS` · `Google Cloud`

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=varshavkumar12345&show_icons=true&theme=tokyonight" height="160px" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=varshavkumar12345&layout=compact&theme=tokyonight" height="160px" />
</p>

---

## Open Source Contribution

- Astropy - https://github.com/astropy/astropy/pull/19181 (closed) :octocat:

---

## 🌐 Connect With Me

<p align="left">
  <a href="https://linkedin.com/in/varsha-v-kumar-1209ba285"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
</p>
